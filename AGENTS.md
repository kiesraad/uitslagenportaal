...
`.github/workflows/frontend-ci.yml`: `biome ci`, vitest, `npm run i18n:check`, `npm run build`
(`tsc -b` + vite), Docker build.

Both run on PRs and both check that generated files are in step with the source, so
