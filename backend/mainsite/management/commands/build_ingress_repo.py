...
A count is uploaded with the osv4-3 CSV `_osv_csv` renders from that same revision, and only its
first upload is `definitieve-documenten`: every one after it is a `correctie`.
"""
...
from mainsite.management.commands._osv_csv import NameKey, read_candidate_names, render_csv
...
- `{prefix}<election id>-tel` holds the counting results under
  ...
  EML document and its osv4-3 CSV. A corrected count is a `correctie_` archive, and the
  Resultaat is zipped under its own name.
"""
...
class Command(BaseCommand):
    ...
    _seed: int = 0
    # Replaced, never updated in place, by `_collect_uploads` with the names on the candidate lists.
    _candidate_names: dict[NameKey, str] = {}
    ...
    def _collect_uploads(self, source: Path) -> tuple[list[Upload], list[Upload], date | None]:
        ...
        The candidate lists also supply the names the osv4-3 CSVs print.
        """
        exchange: list[Upload] = []
        counting: list[Upload] = []
        candidate_names: dict[NameKey, str] = {}
        skipped: list[Path] = []
        ...
        for path in sorted(source.rglob("*.xml")):
            ...
            elif meta.doc_type == EmlType.EML_230b:
                candidate_names.update(read_candidate_names(path))
    ...
    def _uploads(self, meta: EmlMeta, level: str) -> list[Upload]:
        ...
        return [
            Upload(level, organisation, meta.election_id, meta.path, meta.doc_type, revision, corrigenda, self._seed)
    ...
    def _wrap_in_zip(self, upload: Upload, document: bytes, timestamp: datetime) -> bytes:
        """Zip a single EML document under its own name"""
        return make_zip([(upload.path.name, document)], timestamp)

    def _zip_under_own_name(self, root: str, upload: Upload, timestamp: datetime) -> tuple[str, bytes]:
        directory = f"{root}/{upload.level}/{folder_slug(upload.organisation)}"
        return f"{directory}/{eml_stem(upload.path)}.zip", self._wrap_in_zip(upload, upload.read_bytes(), timestamp)
    ...
    def _exchange_file(self, upload: Upload, timestamp: datetime) -> tuple[str, bytes]:
        ...
        return self._zip_under_own_name(EXCHANGE_ROOT, upload, timestamp)

    def _counting_file(self, upload: Upload, timestamp: datetime) -> tuple[str, bytes]:
        """
        Counts arrive as an upload archive holding the zipped EML document and the osv4-3 CSV of
        that same revision. The Resultaat is not uploaded that way, but zipped under its own name.
        ...
        """
        if upload.doc_type == EmlType.EML_520:
            return self._zip_under_own_name(COUNTING_ROOT, upload, timestamp)

        directory = f"{COUNTING_ROOT}/{upload.level}/{folder_slug(upload.organisation)}"
        kind = "correctie" if upload.revision else "definitieve-documenten"
        name = f"{kind}_{upload.election_id.lower()}_{file_slug(upload.organisation)}-{timestamp:%Y%m%d-%H%M%S}.zip"
        document = upload.read_bytes()
        entries = [
            (f"{eml_stem(upload.path)}.zip", self._wrap_in_zip(upload, document, timestamp)),
            (f"osv4-3_telling_{upload.subject.lower()}.csv", render_csv(document, self._candidate_names)),
        ]
