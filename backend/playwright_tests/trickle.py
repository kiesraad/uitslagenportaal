...
def visible_config(*, identifier: str, label: str, category: str) -> ElectionConfig:
...
def expired_config(*, identifier: str, label: str, category: str) -> ElectionConfig:
    return ElectionConfigFactory(
        identifier=identifier,
        label=label,
        category=category,
...
def add_home_timeline(config: ElectionConfig) -> None:
    TimelineEntryFactory(
...
def add_candidate_lists(ws: WsPortal) -> None:
...
def add_borsele_telling(ws: WsPortal) -> None:
...
def add_csb_totaaltelling(ws: WsPortal) -> None:
    assert ws.contest is not None and ws.minderhoud is not None
...
def add_ps_candidate_lists(ps: PsPortal) -> None:
...
def add_aa_en_hunze_telling(ps: PsPortal) -> None:
...
def add_assen_hsb(ps: PsPortal) -> None:
    assert ps.contest is not None and ps.meeuwissen is not None
...
def add_drenthe_csb(ps: PsPortal) -> None:
    assert ps.contest is not None and ps.meeuwissen is not None
